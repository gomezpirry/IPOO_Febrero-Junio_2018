...
void Cuenta::ingresarFondos(){
	system("cls");
	/// EN esta operacion estas usando una variable global, si el usuario no ingresa ningun valor en fondos va a tomar lo que estaba antes alli
	cout<<"\nEscribe la cantidad de fondos que desea ingresar: ";cin>>fondos;
	fondosTotales += fondos;
	cout<<"\nTienes $"<<fondosTotales<<" en total";
	retirar=0; // para que retirar = 0?
	imprimir();
}

void Cuenta::retirarFondos(){
	system("cls");
	/// En esta operacion estas usando una variable global retirar si el usuario no ingresa nada se toma lo que estaba antes alli
